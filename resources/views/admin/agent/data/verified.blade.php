...
@section('main-content')
	...
	<div class="kt-portlet kt-portlet--mobile">
		...
		<div class="kt-portlet__head-actions">
			<button type="button" onclick="window.location.href='{{route('datas.vexport')}}'"  class="btn btn-default btn-icon-sm" ><i class="la la-download"></i> Export
			</button>
		</div>
	</div>
	</div>
		...
		@foreach ($datas as $data)
			<form style="display:none;" action="{{ route('datas.destroy', $data->id) }}" method="post" id="delete-form-{{ $data->id }}">
				@csrf
				@method('DELETE')
			</form>
			<form action="{{route('datas.change_status_unverify')}}" id="change-status-unverify-{{$data->id}}" method="post">
				@csrf
				<input type="hidden" name="type" value="single">
				<input type="hidden" name="entry_id" value="{{$data->id}}">
			</form>
		@endforeach
    <form action="{{route('datas.submit_lot')}}" id="lot-submit-form" method="post">
      @csrf
    </form>
		<div class="kt-portlet__body">
			<div class="row">

			<form id="filter" class="col-xl-12 col-lg-12 col-md-12 col-sm-12" action="{{route('datas.verified')}}" method="post">
				@csrf
			...
			<form action="{{route('datas.change_status_unverify')}}" id="multiple-unverify-form" method="post">
				@csrf
							...
							<td>
								...
								<span class="dropdown">
									...
									<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item" href="{{route('datas.edit', $data->id)}}"><i class="la la-edit"></i> Edit</a>
