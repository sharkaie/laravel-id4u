    ...
    <div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-container  kt-container--fluid ">
            ...
            <div class="kt-subheader__toolbar">
              <a href="{{route('forms.index')}}" class="btn btn-label-dark btn-bold btn-sm btn-icon-h kt-margin-l-10"> << Back </a>
            </div>
        </div>
    </div>
  @endsection
                ...
                @foreach ($fields as $field)
                  <tr>
                    ...
                    <td>{{$field->type}}&nbsp;@if ($field->type=='Options')
                      <a href="{{route('fields.option.index', ['customer_id'=>$id, 'field_id'=>$field->id])}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="View Options">
                        <i class="la la-eye"></i>
                      </a>
                    @endif</td>
                    <td>
                      <form id="delete{{$field->id}}" action="{{route('fields.delete', $field->id)}}" style="display:none;" method="post">
                        @csrf;
                        @method('delete')
                      </form>
                    <a href="#" onclick="
