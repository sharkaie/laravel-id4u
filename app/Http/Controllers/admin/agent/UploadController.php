...
class UploadController extends Controller
{
    ...
    public function excel(){
      if(session('customer_id')){
        ...
        return view('admin.agent.upload.excel', ['user_pass'=>1, 'customers'=>$customers, 'customer'=>$customer]);
      }else{
        $agent_id = Auth::guard('admin')->user()->id;
        $customers = admin_user::where(['role'=>'customer','agent_refered'=>$agent_id])->get();
        return view('admin.agent.upload.excel', ['user_pass'=>0, 'customers'=>$customers]);
      }
    ...
    public function photos(Request $request){
      ...
      if($case == 1){
        return view('admin.agent.upload.photos', ['user_pass'=>1,'search'=>1, 'customer'=>$customer, 'customers'=>$customers, 'search_data'=>$search, 'options'=>$field_options, 'fields'=>$fields, 'data'=>$data]);
      }else{
        return view('admin.agent.upload.photos', ['user_pass'=>1,'search'=>0, 'customer'=>$customer, 'customers'=>$customers,'options'=>$field_options,  'fields'=>$fields, 'data'=>$data]);
      }
        ...
        $agent_id = Auth::guard('admin')->user()->id;
        $customers = admin_user::where(['role'=>'customer','agent_refered'=>$agent_id])->get();
        return view('admin.agent.upload.photos', ['user_pass'=>0, 'customers'=>$customers]);
      }
    }
    ...
    public function photos_cropped(Request $request){
      ...
      if(session('customer_id')){
        ...
        return view('admin.agent.upload.photo_crop_upload', ['user_pass'=>1, 'customer'=>$customer, 'customers'=>$customers]);
      }else{
        $agent_id = Auth::guard('admin')->user()->id;
        $customers = admin_user::where(['role'=>'customer','agent_refered'=>$agent_id])->get();
        return view('admin.agent.upload.photo_crop_upload', ['user_pass'=>0, 'customers'=>$customers]);
      }
